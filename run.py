...
from image_face_fusion.facegan.face_gan import GPEN
from image_face_fusion.facelib.align_trans import (get_f5p, get_reference_facial_points,
                                  warp_and_crop_face,
                                  warp_and_crop_face_enhance)
from image_face_fusion.facelib.damofd import DAMOFD
from image_face_fusion.network.aei_flow_net import AEI_Net
from image_face_fusion.network.bfm import ParametricFaceModel
from image_face_fusion.network.facerecon_model import ReconNetWrapper
from image_face_fusion.network.model_irse import Backbone
from image_face_fusion.network.ops import warp_affine_torch
...
class BBox(object):
    ...
    def reprojectLandmark(self, landmark):
        landmark_= np.asarray(np.zeros(landmark.shape)) 
        for i, point in enumerate(landmark):
            x = int(point[0] * self.w + self.x)
            y = int(point[1] * self.h + self.y)
            landmark_[i] = (x, y)
        return landmark_
...
class ImageFaceFusion(torch.nn.Module):
    ...
    def __init__(self, model_dir: str, *args, **kwargs):
        ...
        self.detector = DAMOFD(model_file=os.path.join(model_dir, 'DamoFD_10g_shape640x640.onnx'))
        self.detector.prepare(ctx_id=0)

        lms_68_model_path = os.path.join(model_dir, 'model_lms.onnx')
    ...
    def detect_face(self, img):
        ...
        det_results, _ = self.detector.detect(img, 0.5, input_size = (640, 640))
        # list, [[x1, y1, x2, y2, score], ...]
        results = []
        if len(det_results) == 0:
            return None, None, None

        box = det_results[0].astype(np.uint32)
        x0 = box[0]
        y0 = box[1]
        x1 = box[2]
        y1 = box[3]

        new_box = BBox([x0,x1,y0,y1])

        cropped_face = img[y0:y1, x0:x1,:]
        cropped_face = cv2.resize(cropped_face, (160, 160))

        input_feed = {}
        input_feed[self.input_node_name[0]] = cropped_face.astype('float32')[np.newaxis,:]
        input_feed[self.input_node_name[1]] = np.array([0]).astype('bool')
        lms_result = self.sess.run(self.out_node_name, input_feed=input_feed)[0][:,:136]
        landmark = lms_result.reshape(-1,2)
        landmarks = new_box.reprojectLandmark(landmark).astype('float32')[np.newaxis, :]
        boxes = np.array([box]).astype('float32')
    ...
    def inference(self, template_img, user_img):
        ...
        landmark_source, _, _ = self.detect_face(user_img_bgr)
        ...
        landmark_template, fw, fh = self.detect_face(template_img_bgr)
...
if __name__ == "__main__":
    model = ImageFaceFusion("./")
    tgt = cv2.imread("./samples/tgt2.png")[:, :, ::-1]
    src = cv2.imread("./samples/src2.png")[:, :, ::-1]

    result = model.inference(tgt, src)
